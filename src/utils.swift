...
//=========== swift Base64编码解码==========
    ...
    func base64Encoding(str:Data)->String

    {
        ...
        let base64String = str.base64EncodedString(options: NSData.Base64EncodingOptions.init(rawValue: 0))
...
func determineTypeAndReturnString(value: Any?) -> String {
    ...
    if let unwrappedValue = value {
        if unwrappedValue is Data {
            ...
            // }
            // if let unwrappedString =  base64Encoding(str: (unwrappedValue as! Data)){
            //     return unwrappedString
            // } else {
            //     return "[Warning] Encoding Shenanigans"
            // }
            let bstring = base64Encoding(str: (unwrappedValue as! Data))

            // return bstring
            if bstring.characters.count > 2000{
                return "[Warning] Encoding Shenanigans"
            }else{
                return bstring
            }
        ...
        switch (unwrappedValue as! String) {
            ...
            case "": return ""
            //default: return base64Encoding(str: (unwrappedValue as! String))
...
func canonicalizeTypesInReturnedDicts(items: [Dictionary<String, Any>]) -> [Dictionary<String, String>] {
    ...
    for eachDict in items {
        ...
        if eachDict[kSecAttrService as String] is Data {
            dict["isService"] = "Data"
        }
        else {
            dict["isService"] = "String"
        }
...
func searchGroup(for query: String, in items: [Dictionary<String, String>]) -> [Dictionary<String, String>] {
    ...
    for eachItem in items {
        ...
        if predicate.evaluate(with: eachItem["Access Group"]) {
            finalItems.append(eachItem)
        }
    }
